...
const LogItem = ({ log, deleteLo, setCurrent }) => {
    const onDelete = () => {
        deleteLog(log.id);
    ...
    return (
        <li className="collection-item">
            <div>
                <a onClick={() => setCurrent(log)} href='#edit-log-modal' className={`modal-trigger ${log.attention ? 'red-text' : 'blue-text'}`}>{log.message}</a>
                <br />
